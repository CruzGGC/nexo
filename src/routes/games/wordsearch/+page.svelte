<script lang="ts">
	...
	import { Search, Calendar, Shuffle, ChevronRight } from 'lucide-svelte';
	...
	const difficulties = [
		{
			key: 'easy' as const,
			label: () => m.sopa_facil(),
			desc: () => m.sopa_facil_desc(),
			color: 'text-accent'
		},
		{
			key: 'medium' as const,
			label: () => m.sopa_medio(),
			desc: () => m.sopa_medio_desc(),
			color: 'text-accent-tertiary'
		},
		{
			key: 'hard' as const,
			label: () => m.sopa_dificil(),
			desc: () => m.sopa_dificil_desc(),
	...
	<div class="relative mx-auto max-w-4xl px-4 lg:px-8">
		...
		<div class="mb-12 text-center">
			<div class="mb-4 inline-flex items-center gap-2">
				<Search class="h-5 w-5 text-accent-tertiary" strokeWidth={1.5} />
				<Badge variant="tertiary" glow>{m.sopa_terminal()}</Badge>
			</div>

			<h1 class="mb-3 font-heading text-4xl font-bold text-foreground md:text-5xl">
				<GlitchText text={m.sopa_titulo()} tag="span" class="text-accent-tertiary cyber-text-glow" />
			</h1>
			<p class="mx-auto max-w-lg font-body text-base tracking-wide text-muted-foreground">
				{m.sopa_subtitulo()}
			</p>
			<div class="mx-auto mt-4 h-px w-24 bg-gradient-to-r from-transparent via-accent-tertiary to-transparent"></div>
		</div>
		...
		<div class="mb-8">
				...
				{#each difficulties as diff}
					<button
						class="cyber-chamfer-sm cursor-pointer border px-5 py-2.5 font-accent text-sm uppercase tracking-wider transition-all duration-200 {selectedDifficulty === diff.key
							? `border-accent-tertiary bg-accent-tertiary/10 ${diff.color} cyber-glow-tertiary`
							: 'border-border text-muted-foreground hover:border-accent-tertiary/50 hover:text-foreground'}"
			...
			<p class="mt-3 font-body text-xs text-muted-foreground">
		...
		<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
			...
			<Card variant="terminal" hoverEffect class="group">
				{#snippet header()}
					<span class="text-accent-tertiary">{m.sopa_diario()}</span>
				{/snippet}
				{#snippet children()}
					<div class="space-y-5">
						<div class="inline-flex h-14 w-14 items-center justify-center border border-accent-tertiary/30 bg-accent-tertiary/5 cyber-chamfer-sm transition-all duration-200 group-hover:cyber-glow-tertiary">
							<Calendar class="h-7 w-7 text-accent-tertiary" strokeWidth={1.5} />
						</div>
						...
						<Button variant="outline" href="/games/wordsearch/play?mode=daily&difficulty={selectedDifficulty}">
							{m.sopa_jogar()}
			...
			<Card variant="terminal" hoverEffect class="group">
				{#snippet header()}
					<span class="text-accent">{m.sopa_aleatorio()}</span>
				{/snippet}
				{#snippet children()}
					<div class="space-y-5">
						<div class="inline-flex h-14 w-14 items-center justify-center border border-accent/30 bg-accent/5 cyber-chamfer-sm transition-all duration-200 group-hover:cyber-glow-sm">
							<Shuffle class="h-7 w-7 text-accent" strokeWidth={1.5} />
						</div>

						<p class="font-body text-sm leading-relaxed text-muted-foreground">
							{m.sopa_aleatorio_desc()}
						</p>

						<Button variant="outline" href="/games/wordsearch/play?mode=random&difficulty={selectedDifficulty}">
							{m.sopa_jogar()}
							<ChevronRight class="ml-1 h-4 w-4" strokeWidth={2} />
						</Button>
					</div>
